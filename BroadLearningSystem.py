...
import numpy as np
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
def BLStrain(train_x,train_y,test_x,test_y,s,c,N1,N2,N3):
#    u = 0
    L = 0
...
#    print('Enhance nodes: max:',np.max(tempOfOutputOfEnhanceLayer),'min:',np.min(tempOfOutputOfEnhanceLayer))
    ...
    #return train_acc_all,train_time
    #测试过程
...
#  强化层输出
    OutputOfEnhanceLayerTest = sigmoid(tempOfOutputOfEnhanceLayerTest * parameterOfShrink)####激活函数 tansig、relu pinv sigmoid
#  最终层输入
...
#  最终测试输出
    ...
    #return train_acc_all,train_time, test_acc,test_time
    return testAcc
...
#  最终测试输出   
    ...
    return test_acc,test_time,Plabel
...
#%---------Training_time1:Accumulative Training Time
   ...
   WARNING: If data comes from a single dataset, the following 'train_xx' and 'train_y1' should be reset!
        '''
        train_xx = preprocessing.scale(train_xf[(20000+(e)*m):(20000+(e+1)*m),:],axis = 1) ####[(10000+(e)*m):(10000+(e+1)*m),:]  584
        train_y1 = train_yf[0:20000+(e+1)*m,:]####[0:10000+(e+1)*m,:]
